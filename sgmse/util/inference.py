...
scheduler="linear"
kwargs = {"path": None, "N": N, "snr": snr, "corrector_step": corrector_steps, "corrector": corrector, "scheduler": scheduler}
...
def evaluate_model(model, num_eval_files, spec=False, audio=False, discriminative=False):
	...
	for i in range(num_eval_files):
		...
		if model.condition == "none":
			kwargs.update(snr=0.2)
			kwargs.update(unconditional_prior=False)
			x_hat = model.enhance(x, **kwargs)
		else:
			x_hat = model.enhance(y, **kwargs)
