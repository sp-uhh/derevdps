...
class Registry:
    ...
    def get_by_value(self, value: str):
        """Get a managed thing by value."""
        for name, val in self._registry.items():
            if val.__name__ == value:
                return name
        raise ValueError(f"{value} did not correpsond to any entry in the registry: here are all entries: {self.get_all_names}")
