...
import h5py
import json
from sgmse.util.other import snr_scale_factor, align, replace_denormals
import torchaudio
	...
	def __init__(
		...
		**ignored_kwargs
		...
		if format in ["wsj0", "vctk", "lj"]:
			...
			self.noisy_files = sorted(glob(join(data_dir, dic_correspondence_subsets[subset]) + '/*.wav')) #not used anyway
...
class SpecsDataModule(pl.LightningDataModule):
	...
	def spec_back(self, spec):
		...
		if self.spec_abs_exponent != 1:
			e = self.spec_abs_exponent
			angle = replace_denormals(spec.angle())
			mag = replace_denormals(spec.abs()**(1/e))
